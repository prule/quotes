...
Provide root-level, agent-oriented guidance (`CLAUDE.md`) so AI agents and contributors can work in this repository correctly without rediscovering its non-obvious conventions each session — the no-build static-site model, the layout, the data sources, and known gotchas.
